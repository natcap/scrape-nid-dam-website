...
def search_state(state_code, work_queue):
    ...
    work_queue.put((bs_table_rows, state_code))
    driver.quit()
...
def main():
    ...
    for state_code in (
        ...
        thread_list.append(
            threading.Thread(
                target=search_state, args=((state_code, work_queue))))
        thread_list[-1].start()
        break

    write_thread = threading.Thread(
        target=writer, args=((TABLE_PATH, work_queue)))
    write_thread.start()
